**(เนื่องจากมีปัญหาเรื่องรูปภาพในระหว่างการแปลงจาก .docx มาเป็น .md)**

**(ถ้าหากต้องการดูเวอร์ชั่นที่มีรูปภาพประกอบโปรดดูใน final_report.pdf)**

![มหาวิทยาลัยเกษตรศาสตร์ -
วิกิพีเดีย](media/image1.png){width="1.0729166666666667in"
...
**นาย ธัญชนก ปั้นมีรส 6510503441**
...
โครงงานนี้สำเร็จลุล่วงได้ด้วยดี คณะผู้จัดทำขอขอบพระคุณ ผู้ช่วยศาสตราจารย์ ดร.ภารุจ
รัตนวรพันธุ์ อาจารย์ประจำวิชา ที่ได้ให้คำแนะนำและแนวทางในการพัฒนาโครงงานมาโดยตลอด

ขอขอบคุณ Zhang et al. (2022) สำหรับบทความวิจัย A Large-scale X-ray Benchmark
for Real-World Prohibited Item Detection (PIDray) (arXiv:2211.10763)
และสำหรับชุดข้อมูล (dataset) PIDray ซึ่งเป็นทรัพยากรข้อมูลหลักที่สำคัญอย่างยิ่งในการฝึกสอน
(train) และประเมินผลโมเดลในโครงงานนี้

ขอขอบคุณ Ultralytics สำหรับ ultralytics library ซึ่งเป็น PyTorch
implementation ที่ช่วยให้การพัฒนา ฝึกสอน และ fine-tuning โมเดล YOLOv11s
เป็นไปอย่างมีประสิทธิภาพ

สุดท้ายนี้ ขอขอบคุณสมาชิกในกลุ่มที่ได้ร่วมมือกันทำงานอย่างเต็มความสามารถ
จนโครงงานนี้สำเร็จลุล่วงได้ด้วยดี

> คณะผู้จัดทำ
>
> นาย ชวัลวิทย์ เกียรติณัฐกร  
> นาย ธัญชนก ปั้นมีรส
...
ในปัจจุบัน การรักษาความปลอดภัยสาธารณะ (public safety)
โดยเฉพาะในพื้นที่ที่มีการสัญจรหนาแน่น เช่น สนามบิน หรือจุดตรวจความปลอดภัย
ถือเป็นสิ่งที่มีความสำคัญอย่างยิ่ง หนึ่งในความท้าทายหลักของภารกิจนี้คือการตรวจจับวัตถุต้องห้าม
(prohibited items) ซึ่งมักถูกซุกซ่อนไว้ในกระเป๋าสัมภาระ

การตรวจสอบภาพ X-ray ด้วยสายตามนุษย์หรือการใช้ระบบ Computer Vision (CV)
แบบดั้งเดิม (Traditional CV) นั้นมีข้อจำกัดหลายประการ ภาพ X-ray มักมีความซับซ้อนสูง
วัตถุมีการปะปน (cluttered) และซ้อนทับกัน (occlusion) ทำให้วิธีการดั้งเดิม เช่น
Template Matching หรือ Feature Descriptors
ทำงานได้ไม่มีประสิทธิภาพเพียงพอต่อความแปรปรวน ทั้งในแง่ของการหมุน (rotation)
หรือการเปลี่ยนแปลงขนาด (scale) นอกจากนี้ วิธีการเหล่านี้ยังต้องการการทำ Feature
Engineering ซึ่งเป็นกระบวนการที่ซับซ้อนและแทบเป็นไปไม่ได้สำหรับข้อมูลภาพ X-ray
ที่มีจำนวนมหาศาล

ด้วยเหตุนี้ เทคโนโลยี Deep Learning (DL) โดยเฉพาะสถาปัตยกรรม Convolutional
Neural Network (CNN) จึงเข้ามามีบทบาทสำคัญ โมเดล CNN ถูกออกแบบมาเพื่อเรียนรู้
features ที่ซับซ้อนเหล่านี้ได้โดยอัตโนมัติ (Automatic Feature Extraction)
ทำให้มีความทนทาน (robust) ต่อความแปรปรวนในภาพได้ดีกว่ามาก

โครงงานนี้จึงมุ่งเน้นการพัฒนา X-ray Prohibited Item Detection Using YOLOv11
โดยมีจุดประสงค์เพื่อประยุกต์ใช้โมเดล State-of-the-Art (SOTA) อย่าง YOLO
ซึ่งเป็นโมเดลแบบ One-Stage Detector ที่มีจุดเด่นด้านความเร็วสูง (real-time
performance)
ซึ่งเป็นปัจจัยสำคัญสำหรับงานที่ต้องการการตัดสินใจที่รวดเร็วอย่างจุดตรวจความปลอดภัย นอกจากนี้
โครงงานนี้ไม่ได้มุ่งเน้นเพียงการ train โมเดล แต่ยังครอบคลุมถึงการสร้างระบบที่ครบวงจร
(end-to-end system) ตั้งแต่การ train model, การสร้าง Backend API
สำหรับให้บริการโมเดล ไปจนถึง Frontend ที่ช่วยให้ผู้ใช้สามารถป้อนภาพและได้รับผลลัพธ์
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image2.png){width="5.052083333333333in"
height="1.53125in"}

2.  เป็นโอกาสในการนำโมเดล Object Detection ที่เป็น State-of-the-Art (SOTA)
    อย่าง YOLO มาประยุกต์ใช้กับปัญหาที่ซับซ้อน

3.  มีการสร้างระบบ End-to-End: project นี้ไม่ได้มีแค่การ train โมเดล
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, แผนที่ เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image3.png){width="6.429187445319335in"
height="6.27163167104112in"}

ภาพตัวอย่างเว็บแอพพลิเคชั่น (<https://x-ray-detection.vercel.app/>)

![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ซอฟต์แวร์, ซอฟต์แวร์มัลติมีเดีย เนื้อหาที่สร้างโดย
AI อาจไม่ถูกต้อง](media/image4.png){width="6.492361111111111in"
height="4.471428258967629in"}
...
<thead>
<tr class="header">
<th>curl --location 'localhost/api/v1/detect'<br />
--form 'file=@"/path/to/file.jpeg"'<br />
--form 'confidence="0.5"'</th>
</tr>
</thead>
<tbody>
</tbody>
...
![รูปภาพประกอบด้วย ข้อความ, แผนภาพ, ภาพหน้าจอ, วางแผน เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image5.jpg){width="7.0541141732283466in"
height="5.083333333333333in"}**4. สถาปัตยกรรม Deep Learning ที่ใช้**
...
dataset\[image_id\] = { \"image\": {filename, width, height},
\"objects\": \[{class_id, bbox}, \...\] }

ทำให้สามารถเรียกดูข้อมูลของแต่ละภาพและกรอบวัตถุได้โดยตรง

![รูปภาพประกอบด้วย ข้อความ, อิเล็กทรอนิกส์, คอมพิวเตอร์, แสดง เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image6.png){width="6.5in" height="5.46875in"}

5.1.2. การสำรวจและสรุปข้อมูลเบื้องต้น

![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ซอฟต์แวร์, ระบบปฏิบัติการ เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image7.png){width="5.916666666666667in"
height="3.136111111111111in"}ใช้ฟังก์ชัน *get_dataset_information()*
เพื่อสรุปจำนวนภาพทั้งหมด จำนวนกรอบวัตถุ และจำนวนภาพที่ไม่มี annotation นอกจากนี้ยังมี
*get_class_distribution()* สำหรับนับจำนวนและสัดส่วนของแต่ละคลาส
เพื่อประเมินความสมดุลของข้อมูล

![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ซอฟต์แวร์, แสดง เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image8.png){width="5.903472222222222in"
...
- ![รูปภาพประกอบด้วย ข้อความ, อิเล็กทรอนิกส์, ภาพหน้าจอ, แสดง เนื้อหาที่สร้างโดย AI
  อาจไม่ถูกต้อง](media/image9.png){width="6.5in"
  height="5.352083333333334in"}ชุด Testing ถูกสุ่มเลือกมา 5,000 ภาพ

![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ซอฟต์แวร์, ตัวอักษร เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image10.png){width="6.5in"
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ซอฟต์แวร์, ตัวอักษร เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image11.png){width="6.5in"
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, คอมพิวเตอร์, ซอฟต์แวร์ เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image12.png){width="7.569444444444445in"
height="5.875in"}ในแต่ละภาพจะมีไฟล์ .txt ที่เก็บ annotation ในรูปแบบ YOLO
พร้อมใช้งานกับ Ultralytics ได้ทันที

5.1.6 การเปลี่ยนชื่อไฟล์ (Rename)

![รูปภาพประกอบด้วย ข้อความ, อิเล็กทรอนิกส์, คอมพิวเตอร์, ภาพหน้าจอ เนื้อหาที่สร้างโดย
AI อาจไม่ถูกต้อง](media/image13.png){width="7.6090277777777775in"
height="5.03125in"}สุดท้ายใช้ฟังก์ชัน *rename_files()*
เพื่อเปลี่ยนชื่อไฟล์ตามระดับความยาก เช่น xray_easy_00001.png,
xray_hard_00002.png, xray_hidden_00003.png
เพื่อให้ง่ายต่อการอ้างอิงและวิเคราะห์ภายหลัง
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ตัวอักษร, เมนู เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image14.png){width="2.1145833333333335in"
height="3.077777777777778in"}ไฟล์ *data.yaml*
ช่วยให้โมเดลรู้ตำแหน่งของข้อมูลสำหรับการฝึก ตรวจสอบ
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ตัวอักษร, เครื่องหมาย เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image15.png){width="2.7430555555555554in"
height="1.0763888888888888in"}โหลดโมเดล YOLOv11s ที่ผ่านการฝึกบน dataset
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, แสดง, ซอฟต์แวร์ เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image16.png){width="3.9775185914260716in"
height="3.492225503062117in"}

![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, แสดง, ตัวอักษร เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image17.png){width="3.145988626421697in"
height="3.3463101487314084in"}
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ตัวอักษร, เครื่องหมาย เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image18.png){width="2.5140190288713913in"
height="1.1275481189851269in"}
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, แสดง, ตัวอักษร เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image19.png){width="4.594099956255468in"
height="2.4423337707786525in"}
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ตัวอักษร, จำนวน เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image20.png){width="2.900761154855643in"
height="1.8529549431321084in"} ![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ,
ตัวอักษร, จำนวน เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image21.png){width="2.895725065616798in"
height="1.8497386264216973in"}

![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ตัวอักษร, ออกแบบ เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image22.png){width="2.8991371391076117in"
height="1.766428258967629in"} ![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ตัวอักษร,
จำนวน เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image23.png){width="2.8034109798775155in"
height="1.7907688101487313in"}
...
- ![รูปภาพประกอบด้วย ข้อความ, ตัวอักษร, ใบเสร็จรับเงิน, ภาพหน้าจอ เนื้อหาที่สร้างโดย
  AI อาจไม่ถูกต้อง](media/image24.png){width="3.6463418635170606in"
  height="1.156411854768154in"}Hidden: ภาพที่มีวัตถุต้องห้ามที่ถูกซ่อนหรือบังอยู่บางส่วน
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ไลน์, เทียน เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image25.jpeg){width="6.5in"
...
โดยสรุป ชุดข้อมูล PIDray
มีจุดเด่นคือความใกล้เคียงกับสภาพแวดล้อมจริงในสนามบินและสถานีขนส่ง ความหลากหลายของฉาก
...
![รูปภาพประกอบด้วย ข้อความ เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image26.png){width="6.5in"
height="6.459722222222222in"}

ภาพกลุ่มตัวอย่าง 16 รูปที่ลองทำการประเมินเชิงคุณภาพ

จากภาพจะเห็นว่าโมเดลสามารถตรวจจับวัตถุได้อย่างถูกต้องและแม่นยำเป็นส่วนใหญ่
แม้ในกรณีที่วัตถุมีการซ้อนทับหรือมีสิ่งของอื่นบดบังบางส่วน
...
โมเดลถูกทดสอบกับชุดข้อมูล 4 กลุ่ม ได้แก่ easy, hard, hidden และ overall
โดยผลลัพธ์สรุปในตารางด้านล่าง

![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, ตัวอักษร, จำนวน เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image27.png){width="5.417422353455818in"
...
ซึ่งเป็นกรณีที่วัตถุถูกซ่อนหรือบังบางส่วน มีความยากสูง จึงได้ค่าความแม่นยำต่ำกว่าที่ประมาณ
65.4%
...
![รูปภาพประกอบด้วย ข้อความ, ภาพหน้าจอ, จำนวน, ตัวอักษร เนื้อหาที่สร้างโดย AI
อาจไม่ถูกต้อง](media/image28.png){width="6.5in"
